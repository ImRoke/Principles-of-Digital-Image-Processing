...
def custom_median_blur(image_path, kernel_size):
    # Read the image
    ...
    for row in range(h - kh + 1):
        for col in range(w - kw + 1):
            for ch in range(c):
                blurred_image[row, col, ch] = np.median(image[row:row + kh, col:col + kw, ch] * kernel)
...
if __name__ == '__main__':
    ...
    cv2.imshow('Custom Median Blur', blurred_image)
    cv2.waitKey(0)
